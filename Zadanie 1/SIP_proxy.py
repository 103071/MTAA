...
def log_calls(call: str):
    log = open("Call Log.log", "a")
    log.write(call + "\n")
    log.close()
...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    def checkValidity(self, uri):
        ...
        if validity > now:
            ...
            del registrar[uri]
            logging.warning("Registration for %s has expired, try again" % uri)
            return False
    ...
    def processRegister(self):
        ...
        if rx_invalid.search(contact) or rx_invalid2.search(contact):
            if fromm in registrar:
                del registrar[fromm]
            self.sendResponse("488 Not Acceptable Here (Nie je akceptovane)")
            return
    ...
    def processInvite(self):
        logging.debug("-----------------")
        logging.debug(" INVITE received (Pozvanka priajta) ")
        logging.debug("-----------------")
        origin = self.getOrigin()
        if len(origin) == 0 or not registrar:
            self.sendResponse("400 Bad Request (Zla poziadavka)")
            return
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                self.sendResponse("480 Temporarily Unavailable (Docasne nedostupny)")
        else:
            self.sendResponse("500 Server Internal Error (Vnutorna chyba siete)")

    def processAck(self):
        logging.debug("--------------")
        logging.debug(" ACK received (prijaty) ")
        logging.debug("--------------")
        destination = self.getDestination()
        if len(destination) > 0:
            logging.info("destination %s" % destination)
    ...
    def processNonInvite(self):
        logging.debug("----------------------")
        logging.debug(" NonInvite received  (prijata) ")
        logging.debug("----------------------")
        origin = self.getOrigin()
        if len(origin) == 0 or not registrar:
            self.sendResponse("400 Bad Request (Zla poziadavka)")
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                self.sendResponse("406 Not Acceptable (Nie je akceptovatelne)")
        else:
            self.sendResponse("500 Server Internal Error (Vnutorna chyba siete)")
        ...
        if len(origin) > 0:
            ...
            if origin in registrar:
                ...
                if "603" in data[0]:
                    ...
                    log_calls(str(date_time) + "Hovor odmietnuty z " + self.getDestination() + " do " + self.getOrigin())
                text = "\r\n".join(data).encode("latin-1")
                socket.sendto(text, claddr)
                showtime()
                logging.info("<<< %s" % data[0])
    ...
    def processRequest(self):
        ...
        if len(self.data) > 0:
            ...
            if rx_register.search(request_uri):
                ...
                now = datetime.now()
                date_time = now.strftime("%d/%m/%Y, %H:%M:%S")
                log_calls(str(date_time) + "Volanie z " + self.getOrigin() + " do " + self.getDestination())
                ...
                now = datetime.now()
                date_time = now.strftime("%d/%m/%Y, %H:%M:%S")
                log_calls(str(date_time) + "Ukonceny hovor z " + self.getOrigin() + " do " + self.getDestination())
