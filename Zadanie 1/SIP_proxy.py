    ...
    def processCode(self):
        ...
        if len(origin) > 0:
            ...
            if origin in registrar:
                ...
                if "603" in data[0]:
                    log_calls("Hovor odmietnuty z " + self.getDestination() + " do " + self.getOrigin())
                text = "\r\n".join(data).encode("latin-1")
                socket.sendto(text, claddr)
                showtime()
                logging.info("<<< %s" % data[0])
    ...
    def processRequest(self):
        ...
        if len(self.data) > 0:
            ...
            if rx_register.search(request_uri):
                ...
                self.processInvite()
                log_calls("Volanie z " + self.getDestination() + " do " + self.getOrigin())

            elif rx_ack.search(request_uri):
                self.processAck()
            elif rx_bye.search(request_uri):
                self.processNonInvite()
                log_calls("Ukonceny hovor z " + self.getDestination() + " do " + self.getOrigin())
